...
class TestUnrollLoopsVarDistinctness:
    def test_unrolled_assign_vars_are_distinct_objects(self):
        @pl.program
        class Prog:
            @pl.function
            def main(self, x: pl.Tensor[[64], pl.FP32]) -> pl.Tensor[[64], pl.FP32]:
        ...
        after = passes.unroll_loops()(Prog)
        func = list(after.functions.values())[0]

        assigned_ids = []

        def _collect(stmt):
            if hasattr(stmt, "var_"):
                assigned_ids.append(id(stmt.var_))
            for child in getattr(stmt, "stmts_", []):
                _collect(child)

        _collect(func.body)

        assert len(assigned_ids) == len(set(assigned_ids)), (
            "Unrolled Var objects must be distinct (no pointer reuse)"
        )
